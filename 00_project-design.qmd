...
\`\`\`{r} \# Logistic Regression 1: Diabetes + Hypertension model_Diabetes_Hypertension \<- glm( COVID_Death \~ Diabetes + Hypertension, data = covid19A, family = binomial() )
...
model_Hypertension_Obesity \<- glm( COVID_Death \~ Hypertension + Obesity, data = covid19A, family = binomial() ) \# Results Table
...
extract_pair_results \<- function(model, model_name) { tidy(model) %\>% filter(term != "(Intercept)") %\>% \# remove intercept mutate( Model = model_name, Odds_Ratio = exp(estimate), P_value = p.value ) %\>% select( Model, Predictor = term, Estimate = estimate, Odds_Ratio, P_value ) }
