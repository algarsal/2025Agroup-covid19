...
#Mean values of expected and observed 
mean_values <- plot_data %>%
  group_by(Comorbidity, Type) %>%
  summarise(
    mean_value = mean(SE_Expected, na.rm = TRUE),
    .groups = "drop"
  )

plot_data %>%
  arrange(Comorbidity, Type) %>%
  select(Comorbidity, Type, SE_Expected)
mean_values
...
mean_table %>%
  ...
  tab_caption("Mean expected and observed values by comorbidity")
mean_table
```
